...
class BaseModel(nn.Module):
    ...
    def to_huggingface(
        ...
        try:
            ...
            raise ImportError("Please install huggingface_hub to use this function.") from e

        hf_hub.create_repo(repo_id=repo_id, token=token, private=private, exist_ok=exist_ok, repo_type="model")

        with tempfile.TemporaryDirectory() as tmpdirname:
            self.to_folder(tmpdirname)
            api = hf_hub.HfApi()
            api.upload_folder(folder_path=tmpdirname, repo_id=repo_id, repo_type="model", token=token, commit_message=commit_message)
